...
import { User, Lock, Eye, EyeOff, Gamepad2, AlertCircle, Info, Phone, Shield } from 'lucide-react'
...
function RegisterForm() {
  ...
  const [form, setForm]         = useState({ username: '', password: '', confirm: '', phoneCode: '+1', phone: '' })
  const [showPass, setShowPass] = useState(false)
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      if (!data.displayName?.toLowerCase().includes('dl')) {
        setRobloxError('Tu nombre de Roblox debe contener "DL". Ej: DL_TuNombre, JohanDL...'); return
      }
      setRobloxData(data)
    } catch { setRobloxError('Error de conexión. Intenta de nuevo.')
  ...
  const handle = async (e) => {
    ...
    setLoading(true)
    const res = await register({ username: form.username, password: form.password, robloxUsername: robloxInput.trim(), phone: `${form.phoneCode}${form.phone.replace(/\s/g,'')}` })
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2.5 p-3 bg-amber-950/30 border border-amber-800/30 rounded-xl">
        ...
        <p className="text-xs text-zinc-400 leading-relaxed">
          Solo miembros de <span className="text-amber-400 font-semibold">Dark License</span> pueden registrarse.
          Tu nombre de Roblox debe contener <span className="text-white font-bold">DL</span>.
        </p>
      </div>
      <form onSubmit={handleVerify} className="space-y-3">
        ...
        <button type="submit" disabled={verifying}
          ...
          {verifying ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin"/>Verificando en Roblox...</span> : '🔍 Verificar username →'}
        </button>
      </form>
    </div>
  )
  ...
  return (
    <form onSubmit={handle} className="space-y-3">
      ...
      <GoldInput label="Contraseña" placeholder="••••••••" type={showPass?'text':'password'}
        ...
        rightIcon={<span onClick={()=>setShowPass(p=>!p)}>{showPass?<EyeOff size={15}/>:<Eye size={15}/>}</span>} required/>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className={`flex bg-[#0d0b08] border rounded-xl overflow-hidden transition-all
          ${errors.phone ? 'border-red-500/60' : 'border-amber-900/40 focus-within:border-amber-500/60 focus-within:shadow-[0_0_12px_rgba(245,158,11,0.1)]'}`}>
